	...
	//定义字符串用于存储sql字符串
		...
		public void add(String f2,String f3,String f4,String f5,
			...
			//为了插入int数据，进行类型转换-Integer.parseInt(field1)
				...
				sql="insert into Histrjnl(JourNo,FromAcc,OldInfo,NewInfo,ChgTime,RegDate,PersonID) values("+getId()+ ",'"+field2+"','"+field3+"','"+field4+"',"+Integer.parseInt(field5)+",'"+field6+"',"+Integer.parseInt(field7)+")";
			...
			public String[] search(String f1) {
				...
				sql="select * from Histrjnl where JourNo="+field1;
				try{
					DB.openConn();//连接数据库
			...
			public String[] searchPI(String f7) {
				...
				//创建字符串数组 用于记录查询的数据
				String[] s =new String[6];
				sql="select * from Histrjnl where PersonID="+field7;
				try{
					DB.openConn();//连接数据库
					//执行查询，查询结果付给rs，rs为结果集
					rs=DB.executeQuery(sql);
			...
			public int getId() {
				...
				sql="select max(JourNo) from Histrjnl";
				try {
					DB.openConn();
					rs=DB.executeQuery(sql);
					if (rs.next()) {
			...
			public String[][] searchAll() {
				...
				int i=0;//循环变量
				//查询sql字符串
				sql="select * from Histrjnl order by JourNo";
				try {
					DB.openConn();
					rs=DB.executeQuery(sql);
						if (rs.last()) {
